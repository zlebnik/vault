---
name: maestra-tasks
description: Use when the user asks to add, update, or roll over tasks in maestra/tasks.md, mentions "tasks" or "todo" in the vault, asks for a daily rollover/cleanup of completed Maestra tasks, or asks to "check Slack threads" / "scan Slack" / "check ClearFeed" / "update task list from Slack".
---

# Maestra Tasks

Personal task list in `maestra/tasks.md`. Daily rollover moves completed `[x]` items into `maestra/completed/YYMMDD.md`. Sources of new tasks: **Slack** (see "Scanning Slack") + **ClearFeed assigned tickets** (see "Checking ClearFeed"). When user runs "scan" / starts the day — sweep BOTH and reconcile into one list.
...
### Checking ClearFeed for assigned tickets

Trigger: every "scan slack" / "wrap up new day" / explicit "check ClearFeed". ClearFeed surfaces support tickets that may not have a Slack mention to Gleb (or where the mention is buried in a channel scan).

**One call, then jq-filter:**
1. `mcp__claude_ai_ClearFeed__requests_list` with `state=["open","in_progress","on_hold","pending"]`, `limit=100`, `filter_by=updated_at`, `sort_order=desc`. The result is large — it gets auto-saved to a tool-results file.
2. Run `jq '.requests[] | select(.assignee.id == "U085GRRHC3V") | {key: .tickets[0].key, title: .title[0:120], state, priority, channel: .channel.name, updated: .updated_at, url: .request_thread.url}'` against that file. Output is a flat list of CF tickets assigned to Gleb.

**Reconcile against `tasks.md` + recent `completed/*.md`:**
For each CF ticket from the jq result:
- **Already in `tasks.md`** (search by `CF-NNNN` substring or by `url`): skip — already tracked.
- **Already in `completed/*.md`** within last ~14 days: work is done in Slack but Gleb didn't close in ClearFeed UI yet. Surface as a one-liner reminder — don't re-add as active task.
- **Not in either**: read the thread (`slack_read_thread`) before adding. Apply the same triage rules as the Slack scan (assignee = Gleb but last message Solved → skip; explicit ask → active; question awaiting reply → waiting).

**Cross-reference for shared root causes.** Two tickets with similar symptoms (e.g. "edits not saving" on different forms) may be one backend bug — flag with `_(связано с CF-MMMM)_` in the task body so future-you knows to fix once.
...
- **Don't skip ClearFeed на scan.** Slack scan ловит mentions последних N дней — а CF держит давно assigned тикеты которые могут уже месяцами висеть. Третий fail-mode: пропустили CF-NNNN assigned to Gleb, нашли его только когда CSM пинганул повторно. Каждый «scan» = Slack + ClearFeed.
